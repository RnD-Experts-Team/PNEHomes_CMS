...
export default function NavigationLinkEdit({ link }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
            { title: 'Layout & Settings', href: '/admin/layout' },
        { title: 'Navigation Links', href: '/admin/navigation-links' },
        { title: 'Edit', href: '#' },
    ];
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/admin/navigation-links/${link.id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit Navigation Link - ${link.title}`} />
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <h1 className="text-2xl font-bold">Edit Navigation Link</h1>
