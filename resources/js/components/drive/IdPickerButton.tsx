        ...
        try {
            if (multiple) {
                const files: PickResult[] = await pickDriveFiles(
                    ...
                    { multiple: true, title: 'Select images' },
                );
                if (files.length && onPickMany)
                    onPickMany(files.map((f) => f.id));
            } else {
            ...
            console.error(e);
            alert(
                'Google Drive Picker failed to open. Check API key/client ID and OAuth consent screen.',
            );
        }
    };
